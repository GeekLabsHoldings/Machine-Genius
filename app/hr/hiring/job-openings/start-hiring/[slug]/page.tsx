...
export default function Page({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    textRef.current = textRef.current.slice(0, data.details?.length || 0);
    textContent.current =
      data.details?.map((item: any) => item.description) || [];
  ...
  const handleOnChange = (e: any, index: number) => {
    // console.log("-----------", index, "-----------");
    // console.log("||||||||||||||||||||||||||||||||||||||||||||");
    // console.log("-----------", textRef.current, "-----------", data.details);
    // if (textRef.current[index]) {
    //   console.log("--------------------------------------------");
    //   const newArr = [...arrText];
    //   newArr[index] = textRef.current[index]!.innerHTML;
    //   console.log("newArr", newArr);
    //   setArrText(newArr);
    // }

    console.log("textRef.current", textRef.current[index]?.innerHTML);
  ...
  const handleEdit = () => {
    if (isEditing) {
      ...
      updatedData.details.forEach((item: any, index: number) => {
        item.description = arrText[index];
      });
      setData(updatedData);
    }
  ...
  const handleBlur = () => {
    const newArr = [...arrText];
    textContent.current.forEach((item, index) => {
      newArr[index] = item;
    });
    setArrText(newArr);
  };
          ...
          <div className="space-y-4">
            ...
            <div className="border border-[#2A2B2A] px-[1.5vw] py-[1.5vh] rounded-[11px] w-1/2 flex justify-between items-center">
              ...
              <div className="flex items-center">
                <CustomCheckBox accentColor="#5FA85B" checked />
              </div>
            </div>
          </div>
