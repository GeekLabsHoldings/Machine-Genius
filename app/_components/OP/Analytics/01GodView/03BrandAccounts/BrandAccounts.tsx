...
export default function BrandAccounts({
  ...
  function AccordionItem({
    ...
    brandData: IBrandPlatformSubscribers;
    option: number;
  }) {
    const brandName = brandIdMap[brandData.brand];
        ...
        <div className="collapse-content space-y-1">
          {Object.entries(platforms).map(([platformName, platformData]) => (
            <div className="flex justify-between" key={platformName}>
              <p>{platformName[0] + platformName.slice(1).toLowerCase()}</p>
              <p className="flex items-center">
                <span>{formatSubscribers(platformData.totalSubscribers)}</span>
                {sidewaysIcon}
              </p>
            </div>
          ))}
        ...
        <div>
          ...
          fetchedTotalSubscribers.length > 0 ? (
            fetchedTotalSubscribers.map((item, index) => (
              <AccordionItem key={index} brandData={item} option={index + 1} />
            ))
          ) : (
