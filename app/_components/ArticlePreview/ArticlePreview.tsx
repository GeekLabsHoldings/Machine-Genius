...
const ArticlePreview = ({ withEdit, isEditable=false, beginSelect, height, yourNewArticle, finalArticle }: IArticleProps) => {
  ...
  return (
    ...
    <div className={` ${styles.articlePreview} ${height} `}>
      <div className={`${styles.articlePreviewData} `}>
        ...
        {yourNewArticle ? <div className={`${styles.articleContent} `}> {finalArticle && finalArticle[0]?.content} </div> : <div>{renderArticleData}</div>}
      </div>
    </div>
