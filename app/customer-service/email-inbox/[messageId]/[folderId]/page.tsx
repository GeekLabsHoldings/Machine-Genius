...
function Page({ params }: { params: { messageId: string; folderId: string } }) {
  ...
  async function getEmailDetails() {
    console.log(`${process.env.NEXT_PUBLIC_API_BASE_URL}/customer-service/email/email-by-id/${params.messageId}/${params.folderId}?brand=${brand}`);
  ...
  return (
    <div className="h-[85vh] py-[1.5vw]">
      <header>
        <div
          className={`flex items-center gap-[--50px] ${styles.email__title}`}
        >
          <div
      ...
      <div className="w-full flex justify-end mt-4">
        <CustomBtn
          word="Reply"
          btnColor="black"
          href={`/customer-service/email-inbox/${params.messageId}/${params.folderId}/reply?brand=${brand}`}
          paddingVal="py-[--10px] px-[--42px]"
        />
      </div>
      <div className="p-8 text-xl">
        <div className="mb-11 emailContent h-[50vh] overflow-y-auto">
