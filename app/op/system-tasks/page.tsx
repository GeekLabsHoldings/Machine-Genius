...
const Page = () => {
  ...
  async function getTasks() {
    try {
      ...
      if (!res.ok) {
        ...
        console.error("Error getTasks:");
        return;
      }
      const json: ITask[] = await res.json();
      if (!json) {
        toast.error("Something went wrong!");
        return;
  ...
  return (
    <div className={`${styles.tasks}`}>
      <div className="flex items-end justify-between py-[--sy-20px]">
        ...
        <CustomBtn
          ...
          href="/op/system-tasks/new-task"
        />
      </div>

      <div className={`${styles.tableContainer} h-[75vh]`}>
        <div className={styles.table}>
          <ul className={styles.table_header}>
            <li className="w-[20%]">User</li>
            <li className="w-[40%]"> Task</li>
            <li className="w-[10%]">Brand</li>
            <li className="w-[10%]">Department</li>
            <li className="w-[20%]">Date Created</li>
          </ul>
          ...
          <div className={styles.table_body}>
            ...
            ) : Array.isArray(pageState.fetchedTasks) &&
              pageState.fetchedTasks.length > 0 ? (
              pageState.fetchedTasks.map((e) => (
                <ul className="w-[100%]" key={e._id}>
                  <li className="w-[20%]">
                    <span
                      className={`text-white px-[0.4vw] py-[0.2vw] rounded-[--3px]`}
                      style={{
                  ...
                  <li className="w-[40%]">{truncateText(e.title, 70)}</li>
                  <li className="w-[10%]">
                    <span
                      className={`text-white px-[0.4vw] py-[0.2vw] rounded-[--3px]`}
                      style={{
                  ...
                  <li className="w-[20%]">{e.start.split("T")[0]}</li>
                </ul>
              ))
            ) : (
              <div className="flex items-center justify-center h-[75vh]">
