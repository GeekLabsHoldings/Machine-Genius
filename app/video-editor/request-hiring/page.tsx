...
export default function page() {
  ...
  function getTitleValue(value: string , data:any) {
    setRequestHiringData({
      ...requestHiringData,
      title: value,
      _id: data?.filter((role:any) => role.roleName == value)?._id
    });
  }
  ...
  async function getAllRoles() {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/hr/role/getByDepartment/${DepartmentEnum.VideoEditing}`,
      {
        method: "GET",
        headers: {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/admin/hiring-request`,
        {
          ...
          body: JSON.stringify({
            ...
            department: DepartmentEnum.VideoEditing,
          }),
        }
      );
      if (res.status === 401) {
